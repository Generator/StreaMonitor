...
class Bot(Thread):
    ...
    def status(self):
        message = self.status_messages.get(self.sc) or self.status_messages.get(
            Status.UNKNOWN
        )
        ...
        if os.path.isdir(videos_folder):
            try:
                for file in os.scandir(videos_folder):
                    if file.is_dir():
                        continue
                    if not os.path.splitext(file.name)[1][1:] in [
                        "mp4",
                        "m4v",
                        "mkv",
                        "webm",
    ...
    def run(self):
        while not self.quitting:
            ...
            offline_time = (
                self.long_offline_timeout + 1
            )  # Don't start polling when streamer was offline at start
            while self.running:
                try:
                    self.recording = False
                    ...
                    if self.sc == Status.OFFLINE:
                        ...
                        if self.sc == Status.PUBLIC:
                            if (
                                self.cookie_update_interval > 0
                                and self.cookieUpdater is not None
                            ):

                                def update_cookie():
                                    while (
                                        self.sc == Status.PUBLIC
    ...
    def getWantedResolutionPlaylist(self, url):
        try:
            ...
            if selected_source["resolution"][1] != 0:
                frame_rate = ""
                if (
                    selected_source["frame_rate"] is not None
                    and selected_source["frame_rate"] != 0
                ):
    ...
    def progressInfo(self, p):
        if p["status"] == "downloading":
            self.log(
                ...
                + str(
                    round(
                        float(p["downloaded_bytes"]) / float(p["total_bytes"]) * 100, 1
                    )
                )
                + "%"
            )
    ...
    def str2site(site: str):
        ...
        for sitecls in LOADED_SITES:
            if (
                site == sitecls.site.lower()
                or site == sitecls.siteslug.lower()
                or site in sitecls.aliases
            ):
                return sitecls
        return None
...
class RoomIdBot(Bot):
    def __init__(self, username, room_id=None):
        ...
        if (
            self.room_id is None and username.isnumeric()
        ):  # Username might be the room ID
            username_real = self.getUsernameFromRoomId(username)
            if (
                username_real is not None
            ):  # Username might not be the room ID even though it is numeric
